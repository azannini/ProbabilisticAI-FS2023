...
class Model(object):
    ...
    def __init__(self):
        ...
        self.kernel = 6*RBF(length_scale=1e-2, length_scale_bounds=(1e-9, 1e2)) + Matern(length_scale=1e-2, nu=1.5) + WhiteKernel(noise_level=1e-5, noise_level_bounds=(1e-13, 1e-02))
        self.model = GaussianProcessRegressor(n_restarts_optimizer=1, kernel = self.kernel, optimizer = 'fmin_l_bfgs_b', normalize_y= True, random_state=2)
    ...
    def make_predictions(self, test_x_2D: np.ndarray, test_x_AREA: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        predictions[mask] += 1*gp_std[mask]
    ...
    def fitting_model(self, train_y: np.ndarray,train_x_2D: np.ndarray):
        ...
        sampler = sample_without_replacement(train_x_2D.shape[0], train_x_2D.shape[0]//3)

        train_x_2D = train_x_2D[sampler]
        train_y = train_y[sampler]
