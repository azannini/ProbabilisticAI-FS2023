...
class SWAGInference(object):
    ...
    def __init__(
        ...
        deviation_matrix_max_rank: int = 23, #20 : 0.836
        ...
        self.SWAG_mean_squared = {name:param**2 for name, param in self.SWAG_mean.items()}
        self.SWAG_second_moment = {name:param**2 for name, param in self.SWAG_mean.items()}

        #we take each name:param inside SWAG mean squared and SWAG second moment and do the difference. name is unchanged.
        self.SWAG_diag_cov= {name: (self.SWAG_mean_squared[name] - self.SWAG_second_moment[name]) for name in self.SWAG_mean_squared.keys()}
        
        # Full SWAG
        # TODO(2): create attributes for SWAG-diagonal
        #  Hint: check collections.deque
        
        self.D_hat = deque(maxlen=self.deviation_matrix_max_rank)

        self.parameters_sets = deque(maxlen=self.deviation_matrix_max_rank)
        self.parameters_sets.append(self._create_weight_copy())

        self.parameters_mean_sets = deque(maxlen=self.deviation_matrix_max_rank)
        self.parameters_mean_sets.append(self.SWAG_mean)
        

        # Calibration, prediction, and other attributes
        # TODO(2): create additional attributes, e.g., for calibration
        self._prediction_threshold = 0.775  # this is an example, feel free to be creative
    ...
    def update_swag(self) -> None:
        ...
        for name, param in current_params.items():
            ...
            #updating mean taking into account how many times we have updated it to avarage correctly
            self.SWAG_mean[name] = (self.SWAG_mean[name] * (self.current_swag_epoch-1) + param) / (self.current_swag_epoch)

            #updating cov taking into account how many times we have updated it to avarage correctly
            #first we compute the two elements of cov, then we compute the cov by taking difference
            self.SWAG_mean_squared[name] = self.SWAG_mean[name]**2
            self.SWAG_second_moment[name] = (self.SWAG_second_moment[name] * (self.current_swag_epoch-1) + param**2) / (self.current_swag_epoch)
            
            if self.current_swag_epoch == self.swag_epochs:
                self.SWAG_diag_cov[name] = (self.SWAG_second_moment[name] - self.SWAG_mean_squared[name])

        # Full SWAG
        if self.inference_mode == InferenceMode.SWAG_FULL:
            # TODO(2): update full SWAG attributes for weight `name` using `current_params` and `param`
            self.parameters_sets.append(current_params)
            self.parameters_mean_sets.append(self.SWAG_mean)
            print(len(self.parameters_mean_sets))
        if self.inference_mode == InferenceMode.SWAG_FULL and self.current_swag_epoch-1 == self.swag_epochs:

            K = min(self.deviation_matrix_max_rank, len(self.parameters_sets))
            for k in range(K):
                D_i = {name: self.parameters_sets[k][name] - self.parameters_mean_sets[k][name] for name in self.parameters_sets[k].keys()}
                self.D_hat.append(D_i)
    ...
    def fit_swag(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        with tqdm.trange(self.swag_epochs, desc="Running gradient descent for SWA") as pbar:
            ...
            for epoch in pbar:
                ...
                self.current_swag_epoch += 1
                if self.current_swag_epoch % self.swag_update_freq == 0:
                    self.update_swag()
    ...
    def calibrate(self, validation_data: torch.utils.data.Dataset) -> None:
        ...
        self._prediction_threshold = 0.775 #2.0 / 3.0
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_std = self.SWAG_diag_cov[name]
            ...
            if self.inference_mode == InferenceMode.SWAG_FULL:
                ...
                # it should have shape (2, param.size())
                effective_k = min(self.deviation_matrix_max_rank, len(self.D_hat))
                z_2 = torch.randn(effective_k)
                
                D_param = torch.stack([D_i[name] for D_i in self.D_hat], dim=param.dim())
                
                sampled_param = current_mean + (1/np.sqrt(2)) * np.sqrt(current_std) * z_1 
                + (1/np.sqrt(2*(self.deviation_matrix_max_rank-1)))*torch.matmul(D_param, z_2)
